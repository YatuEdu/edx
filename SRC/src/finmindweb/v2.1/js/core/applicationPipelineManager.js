	...
	async initalizeState() {
		...
		if (blocks && blocks.data.length > 0) {
			...
			for(let i = 0; i < blocks.data.length; i++) {
				...
				const blockInfo = await Net.getBlockInfo(blockId);
				...
				this._qAndAManager.blockName = blockInfo.blockName;
				this._qAndAManager.blockDescription = blockInfo.blockDescription;
