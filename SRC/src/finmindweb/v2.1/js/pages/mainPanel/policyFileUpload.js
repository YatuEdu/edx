...
const pageTemplate = `
<div class="modal fade" id="viewfile-1" tabindex="-1">
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" style="max-width: 580px;">
		<div class="modal-content">
			...
			<div class="modal-body px-4">
				<div id="uploadToolbar" class="text-end">
					<button id="uploadBtn" type="button" disabled class="btn btn-outline-secondary btn-sm ms-3">&#43; Upload</button>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" data-bs-dismiss="modal" style="min-width: 5.125rem;">Confirm</button>
...
class PolicyFileUpload {
	...
	statusUploaded() {
		...
		$('#uploadBtn').off('click');
		$('#uploadBtn').click(this.handleSubmit.bind(this));
	}

	async updateFileList() {
		this.#updatedHandler();
		// let ret = await Net.insurancePolicyFileList(credMan.credential.token, this.#policyId);
	...
	async handleSubmit() {
		let ret = await Net.insurancePolicyFileAdd(credMan.credential.token, this.#policyId, this.#fileName, this.#fileSize, this.#fileNameUn);
		if (ret.code==null || ret.code==0) {
